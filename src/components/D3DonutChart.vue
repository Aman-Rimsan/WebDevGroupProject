...
function draw(data) {
  ...
  // Hover via jQuery for DOM manipulation (demonstrating jQuery + D3 together)
  ...
  const legend  = svg.append('g').attr('transform', `translate(${legendX},${size / 2 - (data.length * 11)})`);
  data.forEach((d, i) => {
    legend.append('rect')
      .attr('x', 0).attr('y', i * 22).attr('width', 11).attr('height', 11)
      .attr('rx', 3).attr('fill', colorScale(d.label));
    legend.append('text')
      .attr('x', 17).attr('y', i * 22 + 10)
      .attr('font-size', '11px').attr('fill', mutedCol)
